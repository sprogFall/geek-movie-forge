...
```text
Browser (Vite Console, apps/frontend)
...
说明：前端容器会在镜像构建阶段生成 Vite 静态产物，并由 Nginx 直接托管 `dist`。如果 API 不是通过 `http://localhost:8000` 对浏览器暴露，请先在 `.env` 中设置 `VITE_API_BASE_URL`，再执行 `docker compose up --build`。
...
- `VITE_API_BASE_URL`：前端静态构建时写入的 API 基地址，默认 `http://localhost:8000`
