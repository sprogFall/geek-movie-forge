...
class InMemoryProviderService:
    ...
    def create_provider(self, payload: ProviderConfigCreateRequest) -> ProviderResponse:
    ...
    def update_provider(
        ...
        provider = self.require_provider_record(provider_id)
        if payload.name is not None and payload.name != provider.name:
            self._ensure_unique_name(payload.name, exclude_provider_id=provider_id)
    ...
    def list_providers(self) -> ProviderListResponse:
        items = [provider.to_response() for provider in self._providers.values()]
    ...
    def require_provider_record(self, provider_id: str) -> ProviderRecord:
        provider = self._providers.get(provider_id)
        if provider is None:
    ...
    def _ensure_unique_name(self, name: str, exclude_provider_id: str | None = None) -> None:
